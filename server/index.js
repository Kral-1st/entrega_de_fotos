...
// ─── SECURITY HEADERS ───────────────────────────────────────────────────────
app.use(helmet({
  contentSecurityPolicy: false, // API que sirve JSON e imágenes, no HTML con scripts propios
  crossOriginResourcePolicy: { policy: 'cross-origin' } // permite cargar imágenes desde el dominio del frontend
}))
...
// ─── Descarga ZIP del admin — auth por query token, NO cuenta en métricas ─────
app.get('/admin/projects/:id/download', (req, res, next) => {
  if (!req.headers.authorization && req.query.token) {
    req.headers.authorization = `Bearer ${req.query.token}`
  }
  next()
  ...
  try {
    ...
    if (!project) return res.status(404).json({ error: 'Proyecto no encontrado' })
      ...
      if (photos.length === 0) return res.status(404).json({ error: 'No hay fotos disponibles' })

        streamProjectZip(res, { slug: project.slug, ...project }, photos, { useWatermarked: true })
  } catch (err) {
    console.error(err)
...
// Datos de contacto públicos, configurables por .env
