...
document.addEventListener('DOMContentLoaded', () => {
  ...
  .then(r => r.json())
  .then(cfg => { if (cfg.domain) document.getElementById('footerDomain').textContent = cfg.domain })
  .catch(() => {})
...
function renderGallery() {
  ...
  if (footer && p.code) {
    ...
    <p style="color:var(--text-3);font-size:.75rem">
    ${document.getElementById('footerDomain')?.textContent || ''} &nbsp;·&nbsp;
    <a href="/privacidad.html" style="color:var(--text-3);text-decoration:underline">Aviso de privacidad</a>
    </p>
    `
  }
