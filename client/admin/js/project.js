...
function renderProjectHeader(p) {
  ...
  if (codeEl) codeEl.textContent = p.code || '——'
    ...
    if (slugEl) slugEl.textContent = p.slug || '——'
      ...
      document.getElementById('viewCountVal').textContent = p.view_count ?? 0
      document.getElementById('downloadCountVal').textContent = p.download_click_count ?? 0
...
function setupModals() {
  ...
  if (copySlugBtn) {
    copySlugBtn.addEventListener('click', () => {
      ...
      if (navigator.clipboard && window.isSecureContext) {
        ...
        .then(() => { if (typeof showToast === 'function') showToast('ID copiado', 'success'); })
        .catch(() => fallbackCopy(slug));
      } else {
        fallbackCopy(slug);
      }
    });
  ...
  // Eliminar todas las fotos (conserva el proyecto)
  document.getElementById('deleteAllPhotosBtn').addEventListener('click', async () => {
    const nombre = projectData.name
    if (!confirm(`¿Borrar TODAS las fotos de "${nombre}"? El proyecto se conserva, pero esto no se puede deshacer.`)) return
      if (prompt(`Escribe el nombre del proyecto para confirmar:`) !== nombre) {
        showToast('Nombre no coincide, cancelado', 'error')
        return
      }

      try {
        const res = await apiJSON(`/admin/projects/${projectId}/photos`, { method: 'DELETE' })
        if (res.ok) {
